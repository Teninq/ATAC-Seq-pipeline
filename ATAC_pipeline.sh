...
function usage {
  echo -e "\n$(basename $0) is used to pretreat sra or conduct ATACseq analysis task.\n
  Usage:  sh $0 [options]
  Options:
  ...
  echo -e "\e[36me.g. sh ATAC_pipeline.sh -a accession.txt -d -f S -c 5,3 -b -s -p -r -D -P -H -g\n\e[0m"
}
...
TEMP=$(getopt -a -o ha:df:c:bsprDPHgv -l help,access:,dump,fastp:,cut:,bowtie2,sort,picard,remove,deeptools,peaks,HOMER,graph,version -n "$0" -- "$@")
...
if [[ -e ${ACCESS} ]]; then
    ...
    echo -e "\e[31mERROR: Accession file is missing, please provide.\e[0m" >&2
    exit 1
fi
...
if [[ ${CTRL_SAMTOOLS_FILTER} -eq 1 ]]; then
  ...
  do
    ...
    if [[ ${value} -eq 2 ]]; then
      ...
      echo -e "\e[33m=====finish build index for dedup_${sname}.bam!=====\n\e[0m"
    elif [[ ${value} -eq 1 ]]; then
...
if [[ ${CTRL_PEAK_CALLING} -eq 1 ]]; then
  ...
  do
    echo ${date}
    exist_file $WORK_PATH/07_macs2/${sname}_summits.bed
    value=$?
    if [[ ${value} -eq 2 ]]; then
      echo -e "\e[33m=====start call peaks for ${sname}!=====\e[0m"
      macs2 callpeak --nomodel --nolambda --keep-dup all --call-summits \
                     -B -n ${sname} \
                     -t $WORK_PATH/05_samtools_filter/filtered_${sname}.bam \
                     --outdir $WORK_PATH/07_macs2 \
                       >$LOG_PATH/07_macs2/${sname}.callpeaks.log 2>&1
      echo -e "\e[33m=====finish call peaks for ${sname}!=====\e[0m"
    elif [[ ${value} -eq 1 ]]; then
...
if [[ ${CTRL_TFBS_FINDING} -eq 1 ]]; then
  ...
  do
    echo ${date}
    exist_file $LOG_PATH/08_homer/${sname}.homer.log
    value=$?
    if [[ ${value} -eq 2 ]]; then
      echo -e "\e[33m=====start find Motifs for ${sname}!=====\e[0m"
      mkdir $WORK_PATH/08_homer/${sname}
      findMotifsGenome.pl $WORK_PATH/07_macs2/${sname}_summits.bed hg19 $WORK_PATH/08_homer/${sname} \
      -preparsedDir $WORK_PATH/08_homer >$LOG_PATH/08_homer/${sname}.homer.log 2>&1
      echo -e "\e[33m=====finish find Motifs for ${sname}!=====\e[0m"
    elif [[ ${value} -eq 1 ]]; then
